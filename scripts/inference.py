...
def test_model(cp_path):
    ...
    if args.imgpath == None:
        direc = f"{source_dir}/{args.dir}/" if args.dir[-1] not in ["/", "\\"] else f"{source_dir}/{args.dir}"
        correct = 0
        cnt = 0
        total_time = 0
        total_confidence = 0
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"{source_dir}/predictions/"):
        os.makedirs(f"{source_dir}/predictions/")
    if not os.path.exists(f"{source_dir}/predictions/raw/"):
        os.makedirs(f"{source_dir}/predictions/raw/")


    if args.checkpoint == "all":
        res = pd.DataFrame()
        res_all = pd.DataFrame()
        for d in os.listdir(f"{source_dir}/PetClassifier/"):
            checkpoint = os.listdir(f"{source_dir}/PetClassifier/{d}")
            if len(checkpoint) == 0:
                continue
            for i in checkpoint:
                if ".pth" in i:
                    dat = test_model(f"{source_dir}/PetClassifier/{d}/{i}")
    ...
    res.to_csv(f"{source_dir}/predictions/"+timestamp+".csv")
    res_all.to_csv(f"{source_dir}/predictions/raw/"+timestamp+".csv")
